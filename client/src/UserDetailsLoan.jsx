...
import useDetails from "../library/zustand";
import {
  apiClient,
  GETLOANUSER,
  LOANTRANSCATIONPUSH,
...
const[userDetail,setUserdetail]=useState({userName:"",userBalance:"",userPhone:""});

 const userList = async () => {
   try {
     const res = await apiClient.get(`${LOANUSERTRX}/${id}`);
     setUserdetail({
       userName: res.data.userTrx.name,
       userBalance: res.data.userTrx.balance,
       userPhone: res.data.userTrx.phone,
     });
     setData(res.data.transcation);
   } catch (error) {
     console.log("error at geting loan user trx", error);
   }
 };
  useEffect(() => {
      ...
      <div className="box h-[95vh]  w-[450px] bg-gray-900 flex items-center flex-col border border-white relative ">
        <div className="userDetails w-full h-28 bg-gray-800 flex-col font-bold justify-center px-4 items-center text-white capitalize border-b-2 border-gray-600 ">
          <div className="details w-full h-14 flex  justify-around items-center ">
            <p className="name  "> {userDetail.userName} </p>
            <p className="phone"> {userDetail.userPhone} </p>
            <p className="userType">Loan</p>
          </div>
          <div className="currentBalance w-full h-10 bg-gray-900 flex justify-between items-center  border-gray-100 rounded-md text-sm ">
            {parseInt(userDetail.userBalance) > 0 ? (
              ...
              <div className="grp flex justify-between w-full px-6 ">
                ...
                <p className="amount">
                  ₹{" "}
                  {parseInt(userDetail.userBalance) -
                    2 * parseInt(userDetail.userBalance)}{" "}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="transcationsContainer w-full h-full pb-20 px-4 mt-2 overflow-y-scroll no-scrollbar ">
          {data.map((item, index) => (
            <div
              key={index}
        ...
        {admin && (
          <div className="buttons w-full h-16 absolute bottom-0 bg-gray-900 flex justify-around items-center border-t-2 border-gray-600  text-white  ">
            {/* --------------------Modal-------------------------- */}
...
export default UserDetailsLoan;
